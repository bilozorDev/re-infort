-- Fix search_path for get_products_total_inventory function
-- The empty search_path was causing issues finding tables

DROP FUNCTION IF EXISTS public.get_products_total_inventory(UUID[]);

CREATE OR REPLACE FUNCTION public.get_products_total_inventory(
    p_product_ids UUID[]
)
RETURNS TABLE (
    product_id UUID,
...
BEGIN
    ...
    SELECT 
        p.id AS product_id,
        COALESCE(s.total_quantity, 0)::BIGINT AS total_quantity,
        COALESCE(s.total_reserved, 0)::BIGINT AS total_reserved,
        COALESCE(s.total_available, 0)::BIGINT AS total_available,
        COALESCE(s.warehouse_count, 0)::INTEGER AS warehouse_count,
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed: public.get_products_total_inventory() - search_path set to public';
END $$;